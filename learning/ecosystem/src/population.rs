...
use crate::model::{Body, Clock, Identity, TickLedger};
    ...
    let mut individuals: Vec<_> = world
        ...
        .map(|entity| {
            let id = world
                ...
                .0;
            let grazer = world.get::<Body>(*entity).expect("rostered grazer exists");
...
pub(crate) fn mature(
    clock: Res<Clock>,
    mut individuals: Query<(Entity, &Identity, &Body, &mut Individual)>,
...
pub(crate) fn cleanup_dead(
    ...
    grazers: Query<(Entity, &Body)>,
    mut roster: ResMut<Roster>,
) {
    let dead: BTreeSet<_> = grazers
        .iter()
        .filter(|(_, grazer)| !grazer.alive())
        .map(|(entity, _)| entity)
        .collect();
    roster.grazers.retain(|entity| !dead.contains(entity));
    roster.hunters.retain(|entity| !dead.contains(entity));
    for entity in dead {
