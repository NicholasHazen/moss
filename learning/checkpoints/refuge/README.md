...
cargo +1.93.1 run --offline --locked --example mobile
cargo +1.93.1 fmt --all -- --check
cargo +1.93.1 clippy --offline --locked --all-targets -- -D warnings
```

The package has its own target directory, separate from live-project and
...
[Native hunting contract and observations](HUNTING-NOTES.md)
...
```sh
cargo +1.93.1 run --offline --locked --example hunting
```
...
[Native refuge contract and observations](NATIVE-NOTES.md)
...
comparison from this package directory:

```sh
cargo +1.93.1 run --offline --locked --example refuges
```
