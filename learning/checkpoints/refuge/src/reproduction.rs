...
use crate::model::{Body, Clock, Grazer, Identity, Life, Patch, Terminal};
...
impl ParentView {
    fn new(id: &Identity, grazer: &Body, diet: &Grazer, individual: &Individual) -> Self {
        Self {
            id: id.0,
            reserve_units: grazer.reserve_units,
            life: grazer.life(),
            first_eligible_tick: grazer.first_eligible_tick,
            site: diet.feeding_site,
            individual: *individual,
        }
    }
...
pub(crate) fn resolve_births(
    ...
    spatial: (
        Option<ResMut<MobileState>>,
        Option<Res<crate::refuge::RefugeState>>,
    ),
) {
    let (mut mobile_state, refuges) = spatial;
    ...
    let living = parents
        .iter()
        .filter(|(_, _, grazer, _, _, _, _)| grazer.alive())
        .count();
    ...
    for (entity, id, grazer, diet, individual, actor, position) in parents.iter() {
        let view = current_parent(
            ParentView::new(id, grazer, diet, individual),
            actor,
            position,
            &cells,
            mobile_tick,
    ...
    for group in groups.values_mut() {
        ...
        for pair in group.chunks_exact(2) {
            ...
            let views = entities.map(|entity| {
                let (_, id, grazer, diet, individual, actor, position) =
            ...
            let mobile_child = mobile_state.as_ref().map(|mobile| {
                ...
                let cell = cells[&plan.diet.feeding_site.expect("accepted birth has a patch")];
                (
                    Position(cell),
            ...
            if mobile_child.is_some() {
                plan.diet.feeding_site = None;
            }
            let refuge_child = mobile_child.as_ref().and_then(|(position, _)| {
                refuges
                    .as_ref()
                    .map(|state| state.membership(position.0, clock.tick))
            });
            {
                let [
                    (_, _, mut first, _, mut first_stage, _, _),
                    (_, _, mut second, _, mut second_stage, _, _),
                ] = parents
            ...
            if let Some(membership) = refuge_child {
                child.insert(membership);
            }
            let entity = child.id();
