...
Follow the single-world starting design in `docs/ARCHITECTURE.md`. Use native tests to run the simulation schedule without opening a window. Avoid a second ECS mirror, generic adapters, or unused future modules.
...
Update `README.md` with exact verified setup/run/test commands and their working directories. Add `docs/BUILD_NOTES.md` with the resolved versions, enabled features/backend, actual commands/results, platform/browser coverage, and remaining limitations. This file does not exist until there is real evidence to record.
