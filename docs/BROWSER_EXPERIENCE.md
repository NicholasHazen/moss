...
**Status:** Product interaction direction; bootstrap implements a deliberately small subset.
...
Bevy's official pan-camera and viewport-conversion examples provide starting points, not a complete specification for this interaction. Check the APIs for the chosen release. [R3](RESEARCH.md#r3-camera-and-coordinate-conversion)
...
Browsers commonly throttle timers and stop animation callbacks in hidden tabs. Use the Page Visibility API rather than assuming a continuously running desktop loop. [R6](RESEARCH.md#r6-browser-visibility)
...
Choose simple CSS/HTML or Bevy UI for the first controls based on verified integration cost. Do not add a large frontend framework or a second state model to build three buttons. Announce loading and startup failures in an HTML shell even if the renderer never starts.
...
Bevy provides browser examples for WebGL2 and WebGPU. Start by validating the simpler 2D WebGL2 path for the target environment; do not promise automatic renderer fallback or universal device support. [R2](RESEARCH.md#r2-bevy-browser-rendering) Trunk is the proposed local bundler/server. [R4](RESEARCH.md#r4-trunk)
