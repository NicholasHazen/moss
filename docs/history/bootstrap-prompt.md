...
**Historical:** original bootstrap prompt from September 20, 2026.
The foundation and first maintenance exercise are complete. This prompt is not
current authorization. Use [the continuation prompt](../../prompts/CONTINUE.md)
and [NOW.md](../../NOW.md) for the existing project.
...
Read `AGENTS.md`, `PROJECT_BRIEF.md`, `NOW.md`, and `docs/history/bootstrap.md`. Consult `docs/design/architecture.md`, `docs/design/browser-experience.md`, and `docs/research/README.md` for the technical work. Use `docs/agents/collaboration.md` and `docs/tutorial/context/learning-path.md` to prepare the handoff.
...
If actual subagents are available, you may use at most two bounded helpers for web compatibility, build/test review, or mechanical scaffolding. Use `docs/agents/subagents.md` as a guide. Give them distinct responsibilities and file ownership; you integrate and explain the result.
...
Update the README with the exact verified commands and working directories. Create `docs/development/verification.md` with resolved versions, feature flags, checks, and limitations. Update the decision log only where an actual choice changed the proposed defaults.
