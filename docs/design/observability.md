...
`SimId` is implemented. Separate event IDs remain proposed: current journal
entries contain a tick, participants and an event kind. The numbered event below
illustrates future recording rather than a current journal field.
...
```text
Selected SeekFood because energy 18 was below the start threshold 45.
...
Keep decision inputs distinct from the final state of the tick. In the proposed
foraging loop, a seeking animal can decide at reserve 73, eat 4, and finish at
77 while still Seeking; the next choice sees the replenished reserve. A post-tick
snapshot alone cannot tell us which value the earlier policy read. When that
distinction matters, inspect the actual input in a focused test or a bounded
debug trace. Add lasting history only for an outcome or transition worth retaining;
do not invent an earlier input by subtracting guessed costs from the final value.
...
The earliest retained tick identifies the first surviving entry, not necessarily
the start of complete coverage. With capacity 32, recording 35 events on tick 120
evicts the first three but still leaves tick 120 as the oldest retained tick.
Label that as **first retained event: tick 120**, not **complete since tick 120**.
Future event sequence IDs can identify the retained suffix precisely; timestamps
alone cannot distinguish events within the boundary tick.

Separate retention from collection. The current journal records initialization;
three maintenance ticks can change both animals' reserves from 60 to 57 while
its four entries remain unchanged and its eviction count stays zero. Zero
evictions means no recorded entries were dropped, not that every change was
recorded. Future inspection needs to preserve these meanings:

| Evidence available | What the view may say |
| --- | --- |
| An initialized counter covers every resolved death since run start and is zero. | No recorded deaths in this run. |
| The counter reports deaths whose event details have been evicted. | The total is known; some individual details are unavailable. |
| Death outcomes were not collected for the requested scope. | Uncollected or unknown, rather than a numeric zero. |

Increment whole-run counters from actual resolved outcomes independently of
detail retention. A total eviction count is not a per-kind count: one retained
death and three evicted events does not tell us how many of those three were
deaths. Do not reconstruct the death total by adding all evictions to visible
death rows. This accounting rule belongs to future lifecycle integration; it
does not add counters or death behavior to today's foundation.
...
**Later:** local persistence or richer export when a concrete workflow needs it. IndexedDB is a candidate, not a selected dependency. Browser-held data may be subject to quotas and eviction, so a stored run is not a promise of permanent archival. [R7](../research/README.md#r7-browser-storage)
