...
use crate::model::{
    Body, Clock, Daylight, Grazer, GrazerReading, Identity, Patch, PatchReading, Scenario,
    ScenarioError, Snapshot, Terminal, TickLedger,
};
...
impl CourseWorld {
    ...
    /// Hunter evidence supplements the grazer-only projections in hunting mode.
    pub fn hunt_snapshot(&self) -> Option<crate::HuntSnapshot> {
        crate::hunting::snapshot(&self.world)
    }

    /// None means refuge mode is disabled; enabled empty sites remain observable.
    pub fn refuge_snapshot(&self) -> Option<crate::RefugeSnapshot> {
        crate::refuge::snapshot(&self.world)
    }
    ...
    pub fn snapshot(&self) -> Snapshot {
        ...
        let mut grazers: Vec<_> = roster
            ...
            .map(|entity| {
                ...
                let diet = self.world.get::<Grazer>(*entity).expect("grazer has diet");
                GrazerReading {
                    id: id.0,
                    ...
                    meal_units_per_tick: diet.meal_units_per_tick,
                    feeding_site: diet.feeding_site,
                    life: grazer.life(),
                }
            })
            .collect();
...
fn install_mobile(initial: &MobileScenario, run: u64, next_child_id: crate::SimId) -> World {
    ...
    crate::hunting::install(&mut world, initial);
    crate::refuge::install(&mut world, initial);
    world
}
...
fn mobile_schedule() -> Schedule {
    ...
    schedule.add_systems(
        (
            ...
            (
                mobile::rest::decide,
                crate::hunting::activity::decide,
                crate::refuge::refresh_memberships,
                mobile::perception::observe,
                crate::hunting::perception::observe,
                mobile::perception::choose,
                crate::hunting::perception::choose,
                mobile::movement::travel,
                crate::hunting::movement::travel,
                crate::refuge::refresh_memberships,
                mobile::rest::execute,
                crate::hunting::activity::execute,
                crate::hunting::capture::capture,
