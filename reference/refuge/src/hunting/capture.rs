...
pub(crate) fn capture(
    ...
    for (hunter_id, hunter_entity) in order {
        let (_, _, body, position, _, hunter, actor, _) =
        ...
        let (_, _, prey, prey_position, diet, prey_hunter, _, membership) =
            bodies.get(prey_entity).expect("listed prey");
        if diet.is_none()
            || membership.is_some_and(|membership| membership.refuge.is_some())
            || prey_hunter.is_some()
            || !prey.alive()
        ...
        let [
            (_, _, mut hunter_body, _, _, hunter, mut actor, _),
            (_, _, mut prey_body, _, _, _, _, _),
        ] = bodies
